# UNISA Short Learning Programmes: AI + HRM Portfolio

This repository contains a research-backed portfolio of five proposed Short Learning Programme concepts for the University of South Africa context.

The seed idea is **AI + HRM for Managers: From LLM Use to Work Redesign**. The portfolio expands it into five standalone SLP concepts for a mixed audience of managers, HR practitioners, researchers, postgraduate learners, and applied policy/strategy professionals.

## Repository Map

- [Programme portfolio overview](docs/00-programme-portfolio-overview.md)
- [Rapid scoping review method](docs/01-rapid-scoping-review-method.md)
- [Evidence synthesis matrix](docs/02-evidence-synthesis-matrix.csv)
- [SLP 01: AI + HRM for Managers](docs/slp-01-ai-hrm-managers-work-redesign.md)
- [SLP 01 full programme package](docs/slp-01-programme/README.md)
- [SLP 02: Responsible AI Governance in HRM](docs/slp-02-responsible-ai-governance-hrm.md)
- [SLP 02 full programme package](docs/slp-02-programme/README.md)
- [SLP 03: People Analytics and AI Decision Support](docs/slp-03-people-analytics-ai-decision-support.md)
- [SLP 03 full programme package](docs/slp-03-programme/README.md)
- [SLP 04: Agentic HRM and Workflow Automation](docs/slp-04-agentic-hrm-workflow-automation.md)
- [SLP 04 full programme package](docs/slp-04-programme/README.md)
- [SLP 05: AI, Green HRM and Sustainable Work Transformation](docs/slp-05-ai-green-hrm-sustainable-work.md)
- [Course development roadmap](docs/03-course-development-roadmap.md)
- [Cross-concept research brief](docs/04-cross-concept-research-brief.md)
